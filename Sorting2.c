...
int main() 
{
    ...
    FILE *in = fopen("Knapsack1.txt", "r");//File pointer for file operations
    FILE *out = fopen("output-Sorting-part.txt", "w");//File pointer for file operations
    ...
    float weight[n], cost[n], profit[n], ratio[n];//Arrays to store data in each item
    ...
    for (int i = 0; i < n; i++) 
    {
        fprintf(out, "%-4d\t%-7.2f\t%-7.2f\t%-7.2f\t%-7.2f\n",
            i + 1, weight[i], cost[i], profit[i], ratio[i]);
    }


    fclose(in);//save changes and free memory
    fclose(out);//save changes and free memory
